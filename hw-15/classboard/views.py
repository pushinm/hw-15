...
from classes.models import ClassByDayChoices, ClassPairChoices
...
def filters_students_options(request, pk=None):
    if request.method == 'POST':
        ...
        return Classboard.objects.filter(filter_q).prefetch_related('teacher').select_related('class_name', 'group')

    return Classboard.objects.all().prefetch_related('teacher').select_related('class_name', 'group')
